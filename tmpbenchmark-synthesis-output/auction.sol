contract Auction {
  ...
  function updateWithdrawOnInsertRecv_withdraw_r14(address p,uint n) private   returns (bool) {
      bool b_1 = end.b;
  ...
  function updateHighestBidAmountOnInsertBid_r23(uint n) private    {
      uint _max = highestBidAmount.n;
      if(n>_max) {
        highestBidAmount = HighestBidAmountTuple(n,true);
      }
  }
  function updateBidOnInsertRecv_bid_r13(address p,uint n) private   returns (bool) {
      uint m_1 = highestBidAmount.n;
      bool b_0 = end.b;
      if(b_0!=true && n>m_1) {
        updateBidTotalOnInsertBid_r25(p,n);
        updateHighestBidAmountOnInsertBid_r23(n);
        emit Bid(p,n);
        return true;
      }
      return false;
